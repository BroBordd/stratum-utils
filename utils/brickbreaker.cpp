...
// proper filled rounded rect using 3 rects + 4 corner ellipses
static void drawRoundRect(float x0,float y0,float x1,float y1,
    ...
    // top/bottom strips
    drawRect(x0+rad,  y0,      x1-rad,  y0+rad,  r,g,b,a);
    drawRect(x0+rad,  y1-rad,  x1-rad,  y1,      r,g,b,a);
    // corners via small rects (good enough at this scale)
    drawRect(x0,      y0,      x0+rad,  y0+rad,  r,g,b,a);
    drawRect(x1-rad,  y0,      x1,      y0+rad,  r,g,b,a);
    drawRect(x0,      y1-rad,  x0+rad,  y1,      r,g,b,a);
    drawRect(x1-rad,  y1-rad,  x1,      y1,      r,g,b,a);
}
...
static const float BALL_SPEED  = 0.55f;
...
struct Brick { bool alive; int hits; };
...
struct GameState {
    ...
    void launch(){
        if(launched) return;
        float angle=-1.15f+(rand()%100)/195.f;
        ball.vx=BALL_SPEED*sinf(angle);
        ball.vy=-BALL_SPEED*cosf(angle);
        if(fabsf(ball.vx)<0.08f) ball.vx=ball.vx<0?-0.08f:0.08f;
        launched=true;
    }
    ...
    void update(float now){
        ...
        if(!launched){ ball.x=paddleX; ball.y=PADDLE_Y-BALL_R-0.002f; return; }
        ...
        if(ny-BALL_R<PLAY_AREA_Y0){ ny=PLAY_AREA_Y0+BALL_R; ball.vy=fabsf(ball.vy); }
        ...
        float py0=PADDLE_Y-PADDLE_H*0.5f, py1=PADDLE_Y+PADDLE_H*0.5f;
        float px0=paddleX-PADDLE_W*0.5f,  px1=paddleX+PADDLE_W*0.5f;
        if(ball.vy>0 && ny+BALL_R>=py0 && ny-BALL_R<=py1 && nx>=px0 && nx<=px1){
            ny=py0-BALL_R; ball.vy=-fabsf(ball.vy);
            float rel=(nx-paddleX)/(PADDLE_W*0.5f);
        ...
        for(int r=0;r<ROWS;r++){
            for(int c=0;c<COLS;c++){
                ...
                float overlapL=nx+BALL_R-bx0, overlapR=bx1-(nx-BALL_R);
                float overlapT=ny+BALL_R-by0, overlapB=by1-(ny-BALL_R);
                float minH=fminf(overlapL,overlapR), minV=fminf(overlapT,overlapB);
                if(minH<minV) ball.vx=(overlapL<overlapR)?-fabsf(ball.vx):fabsf(ball.vx);
                else          ball.vy=(overlapT<overlapB)?-fabsf(ball.vy):fabsf(ball.vy);
                bricks[r][c].hits--;
                if(bricks[r][c].hits<=0){
                    bricks[r][c].alive=false;
                    score+=(ROWS-r)*10;
                    float mx=(bx0+bx1)*0.5f, my=(by0+by1)*0.5f;
                    spawnParticles(mx,my,BRICK_COLORS[r][0],BRICK_COLORS[r][1],BRICK_COLORS[r][2]);
                    Stratum::vibrate(14);
                } else { Stratum::vibrate(6); }
                goto done_bricks;
            }
        }
        done_bricks:
        ball.x=nx; ball.y=ny;
        if(allCleared()){ resetBricks(); resetBall(); score+=500; }
    }
};
...
int main(int,char**){
    ...
    s.onTouch([&](const TouchEvent& e){
        ...
        if(e.action==TouchAction::DOWN || e.action==TouchAction::MOVE){
            float newX=fmaxf(PADDLE_W*0.5f+0.01f,fminf(1.f-PADDLE_W*0.5f-0.01f,e.x));
            gGame.paddleX=newX;
            if(e.action==TouchAction::DOWN && !gGame.launched) gGame.launch();
        }
    });
    ...
    s.onFrame([&](float){
        ...
        { std::lock_guard<std::mutex> lk(gGame.mtx); gGame.update(now); }
        ...
        {
            std::lock_guard<std::mutex> lk(gGame.mtx);
            paddleX=gGame.paddleX; ballX=gGame.ball.x; ballY=gGame.ball.y;
            launched=gGame.launched; score=gGame.score; highScore=gGame.highScore;
            flashT=gGame.flashT;
        ...
        float flashA=(flashT>0&&flashAge<0.25f)?(1.f-flashAge/0.25f)*0.35f:0.f;
        ...
        float maxW=0.46f, baseSS=fminf(0.026f,0.55f*asp/10);
        ...
        for(int r=0;r<ROWS;r++){
            for(int c=0;c<COLS;c++){
                ...
                float br=BRICK_COLORS[r][0], bg=BRICK_COLORS[r][1], bb=BRICK_COLORS[r][2];
                float dim=bricks[r][c].hits==2?1.0f:0.55f;
                drawRoundRect(bx0,by0,bx1,by1,br*dim,bg*dim,bb*dim,1.f,0.006f);
                drawRect(bx0+0.006f,by0+0.004f,bx1-0.006f,by0+0.008f,1.f,1.f,1.f,0.18f*dim);
                if(bricks[r][c].hits==1&&r<2){
                    float mx=(bx0+bx1)*0.5f;
                    drawRect(mx-0.002f,by0+0.010f,mx+0.002f,by1-0.010f,0.f,0.f,0.f,0.35f);
                }
            }
        }

        for(auto& p:parts){
            float u=p.life/p.maxLife, sz=0.010f*u;
        ...
        drawEllipse(ballX-brad*0.25f,ballY-brad*0.3f,brad*0.38f,brad*0.38f*asp,1.f,1.f,1.f,0.55f,16);
        ...
        if(!launched){
            ...
            Text::draw("TAP TO LAUNCH",0.5f-6.5f*hs/asp,PADDLE_Y+0.040f,
                       hs,0.55f*pulse,0.75f*pulse,1.00f*pulse);
        }

        float fs2=fminf(0.015f,0.60f*asp/14);
        Text::draw("DRAG  move paddle     PWR  exit",0.05f,0.975f,fs2,0.18f,0.20f,0.28f);
